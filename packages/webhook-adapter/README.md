...
The package has 8/8 tests, a clean install, and a clean TypeScript build in
...
The resolver and HTTP request share the configured timeout. This matters for
custom DNS/endpoint resolvers as well as the built-in client: a resolver that
does not complete cannot hold a worker lease indefinitely. The injected
network fakes in the test suite are deliberately deterministic; no test opens
an external socket.
