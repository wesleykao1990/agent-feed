...
| M5-016 | The first hosted run passed but warned that the Node 20 runtimes in `actions/checkout@v4` and `actions/setup-node@v4` were deprecated. | Verify the current official releases and update checkout, setup-node, and setup-python consistently to v7; retain hosted clean-checkout validation. | Resolved pending replacement run |
