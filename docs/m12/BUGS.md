...
| M12-008 | The first hosted M12 job installed only the new utility packages and persistence package. A clean runner therefore lacked persistence's existing local dependency graph, while the M9 guard rejected the missing job-registry prerequisite. | Install the complete locked persistence dependency chain in the isolated M12 job before running conformance. This keeps every milestone job independently reproducible on a clean runner. |
