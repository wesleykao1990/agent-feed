...
| `apps/delivery-worker` | Process lifecycle, claim loop, leases, transport, signing-key wiring, shutdown, exporter | 10 focused tests; clean install/build; live repository behavior green through combined suite | Bounded process/CLI entrypoint and owner-only signing-key-file resolver are implemented; hosted endpoint/exporter deployment remains operational work. |
