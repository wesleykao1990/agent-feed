...
| LR-008 | A future-only subscription could not receive an explicitly authorized historical recovery set without direct SQL or broadening all history. | Add an exact event/run-set materializer that reapplies selectors transactionally, rejects every mismatch, and is idempotent. |
| LR-009 | The webhook client used only the first validated DNS address, so one unreachable address could fail an otherwise reachable public endpoint. | Try at most four already validated pinned addresses for connection failures before headers, with one overall abort signal and no DNS re-resolution. |
| LR-010 | A temporary webhook hostname expired between materialization and delivery, placing all 21 authorized events into retry. | Keep durable retry state, rotate only the subscription endpoint, and replay the same delivery identities; no rematerialization or new events are needed. |
