...
| M3-027 | A real ChatGPT Scheduled Task output completed its first PostgreSQL import, but replaying the same generated one-batch bundle failed with `terminal_run_immutable`. The existing local-file retry fixture had zero batches and therefore did not exercise this ordering. | Look up the locked run's accepted batch by idempotency key and validate its payload hash before applying terminal immutability. Added direct persistence coverage and a durable ChatGPT adapter regression proving one run, one batch, and one evidence row across an exact full-bundle retry. |
| M3-028 | The first hosted run of the new durable ChatGPT regression failed before assertions because CI ran M1 ingress before installing the ChatGPT adapter's source-linked local-file dependency graph. A populated local checkout masked the ordering gap. | Move the ChatGPT adapter clean install into the M1 dependency phase and remove the later duplicate M3 install; hosted clean-checkout CI is the retained regression. |
