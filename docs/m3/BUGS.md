...
| M3-028 | The first hosted run of the new durable ChatGPT regression failed before assertions because CI ran M1 ingress before installing the ChatGPT adapter's source-linked local-file dependency graph. A populated local checkout masked the ordering gap. | Move the ChatGPT adapter clean install into the M1 dependency phase and remove the later duplicate M3 install; hosted clean-checkout run `32096064685` passed. |
