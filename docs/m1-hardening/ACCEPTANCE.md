...
| M1H-A10 | Full combined gate | Foundation, protocol drift/compatibility, conformance 23/23, prototype 29/29, M1 live ingress 5/5, PostgreSQL 11/11, schema 4/4, producer 9/9, API 2/2, local-file 6/6, and full M2 gate all pass. Hosted run `32056120146` passed on source commit `b217470552d668d6694edfa7e28b15b3279a73f5`. | Passed locally and in hosted PR CI |
...
| M1H-A12 | Final integrity inventory | `npm run checksums:write` followed by `npm run checksums:check` verifies 237 source-file digests after integration; ignored build/release outputs are excluded so clean-checkout CI sees the same set. | Passed locally and in hosted run `32056120146` |
