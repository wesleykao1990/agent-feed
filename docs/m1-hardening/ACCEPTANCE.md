...
|---|---|---|---|
| M1H-A01 | Executable API boundary | `npm run m1:architecture`; clean build/test of `packages/producer-service` and `apps/api` (8/8 and 2/2). Handlers use `ProducerService`; SQL remains in the PostgreSQL adapter. | Passed locally |
...
| M1H-A08 | Immutable artifact candidate | `npm run schema:artifact -- --tag schema-v0.1.1` creates `agent-feed-schema-0.1.1.tgz` plus a manifest. Local candidate SHA-256: `b6c8c6beb98fea305346f4a23f049deba8498beba84fa643d49c9ecfb8adce75`; integrity: `sha512-Gy3pWM0xNwuGxwXTHx9PH1HmqUwevtToAuXPJQ1JLCScn8+PPJ6atJkO+/2txRAWSRPJ63LlTw3sqSvHLeAxHQ==`. | Candidate passed; immutable release URL/source tag pending merge |
...
| M1H-A10 | Full combined gate | Foundation, protocol drift/compatibility, conformance 23/23, prototype 29/29, M1 live ingress 5/5, PostgreSQL 11/11, schema 4/4, producer 8/8, API 2/2, local-file 6/6, and full M2 gate all pass. | Passed locally |
...
| M1H-A12 | Final integrity inventory | `npm run checksums:write` followed by `npm run checksums:check` verifies 236 source-file digests after integration; ignored build/release outputs are excluded so clean-checkout CI sees the same set. | Passed on PR branch |
